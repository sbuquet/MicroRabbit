...
namespace MicroRabbit.Infra.Bus
{
    public sealed class RabbitMQBus : IEventBus
    {
        ...
        private readonly IServiceScopeFactory _servideScopeFactory;

        public RabbitMQBus(IMediator mediator, IServiceScopeFactory servideScopeFactory)
        {
            _mediator = mediator;
            _handlers = new Dictionary<string, List<Type>>();
            _eventTypes = new List<Type>();
            _servideScopeFactory = servideScopeFactory;
        }
        ...
        private async Task ProcessEvent(string eventName, string message)
        {
            if (_handlers.ContainsKey(eventName))
            {
                using (var scope = _servideScopeFactory.CreateScope())
                {
